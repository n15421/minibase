...
int lb_sym_l1_cache_add(const char *in_sym, void *in_addr)
{
	if (g_lb_sym_l1_cache_tab_count < LB_SYM_L1_CACHE_SIZE - 1) {
		...
		printf("[LightBase] [DEBUG] L1 Cache add!: %s, %p.\n", in_sym, in_addr);
...
void sym_cache_downgrade()
{
	...
	qsort((void *)g_lb_sym_l1_cache_tab, g_lb_sym_l1_cache_tab_count, sizeof(struct sym_cache), (_CoreCrtNonSecureSearchSortCompareFunction)cache_qsort);
	qsort((void *)g_lb_sym_l2_cache_tab, g_lb_sym_l2_cache_tab_count, sizeof(struct sym_cache), (_CoreCrtNonSecureSearchSortCompareFunction)cache_qsort);
